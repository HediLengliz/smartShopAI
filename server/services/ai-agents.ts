...
export class NLPAgent {
  ...
  async parseNaturalLanguage(text: string, userId: string): Promise<NLPParseResult> {
    ...
    console.log(`[NLP Agent] Parsed ${items.length} items with confidence ${confidence}`);
   ...
   */
  async parseWithProductMatching(text: string, userId: string): Promise<NLPParseResult> {
...
export class RecommendationAgent {
   ...
   */
  async getPersonalizedRecommendations(userId: string, limit = 4): Promise<RecommendationResult> {
    console.log(`[Recommendation Agent] Getting recommendations for user ${userId}`);
    ...
    try {
      ...
      const availableProducts = allProducts.filter(p => 
        !recentProductIds.has(p._id.toString()) && 
        p.stock > 0
      );
      ...
      console.log(`[Recommendation Agent] Generated ${recommendations.length} recommendations`);
      
      return { products: recommendations };
    } catch (error) {
      console.error('[Recommendation Agent] Error generating recommendations:', error);
      return { products: [] };
    }
  }
   ...
   */
  async getRelatedProducts(productId: string, limit = 4): Promise<RecommendationResult> {
    console.log(`[Recommendation Agent] Getting related products for ${productId}`);
    ...
    try {
      ...
      console.error('[Recommendation Agent] Error getting related products:', error);
      return { products: [] };
    }
  }
  ...
  async getTrendingProducts(limit = 4): Promise<RecommendationResult> {
    ...
    try {
      ...
      console.error('[Recommendation Agent] Error getting trending products:', error);
      return { products: [] };
    }
  }
...
export class ChatbotAgent {
   ...
   */
  async processMessage(userId: string, message: string, history?: Array<{ role: string; content: string }>): Promise<ChatbotResponse> {
    console.log(`[Chatbot Agent] Processing message from user ${userId}: "${message}"`);
    ...
    try {
      ...
      if (matchingFaq) {
        ...
        suggestions.push("Is there anything else I can help with?", "Would you like to see more FAQs?");
        ...
        suggestions.push("Show me products", "How do I add items?", "Create a new list");
        ...
        suggestions.push("View my orders", "Get recommendations", "Browse products");
        ...
        suggestions.push("Create a new list", "Browse products", "View my orders");
      } else if (lowerMessage.includes("payment") || lowerMessage.includes("pay")) {
        response = "We support various payment methods. You can pay for your orders using credit cards, PayPal, or other secure payment options.";
        suggestions.push("View payment options", "Check order status", "Contact support");
      } else if (lowerMessage.includes("recommendation") || lowerMessage.includes("suggest")) {
        response = "Our AI analyzes your shopping history to provide personalized product recommendations. Check the Products page to see your recommendations!";
        suggestions.push("View recommendations", "Browse products", "Create a list");
      } else {
        response = "I understand you're looking for help. I can assist you with shopping lists, products, orders, and payments. What specific topic would you like to know about?";
      ...
      console.log(`[Chatbot Agent] Generated response with ${suggestions.length} suggestions`);
  ...
  async generateSuggestions(context: string): Promise<string[]> {
    console.log(`[Chatbot Agent] Generating suggestions for context: "${context}"`);
    ...
    if (context.includes("list")) {
      suggestions.push("How do I add items to my list?", "Can I share my list?", "How do I delete a list?");
    } else if (context.includes("product")) {
      suggestions.push("How do I search for products?", "Can I filter products by category?", "How do I see product details?");
    } else if (context.includes("order")) {
      suggestions.push("How do I track my order?", "Can I cancel an order?", "How do I reorder items?");
    } else {
      suggestions.push("How do I get started?", "What features are available?", "How do I contact support?");
    }
